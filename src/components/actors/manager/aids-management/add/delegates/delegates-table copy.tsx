...
export default function Delegates_Table({
  ...
  //         ...prev,
  //         existingQuantity:
  //           prev.existingQuantity - portion * allDelegatesData.length,
  //       }));
  //     }
  //   }
  // }, [
  ...
  const handleRowSelection = (id: number | string, checked: boolean) => {
    if (checked) {
      const portion =
        query.delegatesPortions === DELEGATE_PORTIONS.equal
          ? query.delegateSinglePortion
          : selectedDelegatesPortions.find((d) => d.delegate_id === id)
              ?.portion || 0;
      if (
        query.quantityAvailability === QUANTITY_AVAILABILITY.limited &&
      ...
      if (
        ...
        // setQuery((prev) => ({
        //   ...prev,
        //   existingQuantity: prev.existingQuantity - portion,
        // }));
      }
      ...
      if (
        ...
        setQuery((prev) => ({
          ...prev,
          existingQuantity: prev.existingQuantity + removed.portion,
        }));
      }
      setSelectAllAcrossPages(false);
    }
    ...
    if (selectAll) {
      ...
      if (
        ...
        if (
          ...
          setRemainingQuantity(
            query.existingQuantity -
              portion * (delegatesData.pagination.totalItems || 0) // or allDelegatesData?.length
          );
        }
      }
    } else {
  ...
  //       if (exists) {
  //         return prev.map((d) =>
  //           d.delegate_id === delegateId ? { ...d, portion: 0 } : d
  //         );
  //       }
  //       return prev;
  //     });
    ...
    <Table.Tr>
      ...
      >
        <Checkbox
          ...
          onChange={(event) => {
            // console.log('🚀 ~ selectAll:', event.currentTarget.checked);
