...
export default function Aid_Delegates_Table_COPY({
  destination,
  ...
  useEffect(() => {
    if (query.delegatesPortions === DELEGATE_PORTIONS.MANUAL) {
      // ensure all selected delegates have an entry with portion
      ...
      // sync all portions to delegateSinglePortion
      setSelectedDelegatesPortions((prev) => {
        return prev.map((item) => ({
          delegate_Id: item.delegate_Id,
  ...
  const {
    ...
    queryKey: ['add_delegates', currentPage, destination],
  ...
  const isRowSelected = useCallback(
    (id: number) => selectedDelegatesIds.includes(id),
  ...
  const handleRowSelection = useCallback(
    ...
    [remainQ, query, setSelectedDelegatesPortions]
  );
  ...
  const rows = useMemo(
    () =>
      (delegatesData?.delegates || []).map((delegate, index) => (
        ...
        >
          ...
          >
            <Checkbox
              checked={isRowSelected(delegate.id)}
              onChange={(e) =>
            ...
            <NumberInput
              ...
              onChange={(val) => {
                if (query.delegatesPortions === DELEGATE_PORTIONS.MANUAL) {
                  ...
                  setSelectedDelegatesPortions((prev) => {
                    // Remove delegate if portion becomes 0
                    if (value === 0) {
                      return prev.filter(
                    ...
                    // Calculate remaining quantity excluding this delegate’s current portion
                    const currentPortion =
                      prev.find((item) => item.delegate_Id === delegate.id)
                        ?.portion || 0;
                    const totalWithoutThis =
                      totalSelectedPortion - currentPortion;
                    ...
                    if (newTotal > query.existingQuantity) {
                      ...
                      // Remove the delegate since portion can’t be set properly
                      return prev.filter(
                        (item) => item.delegate_Id !== delegate.id
                      );
                    }

                    // If delegate already exists, update their portion
                    if (prev.some((item) => item.delegate_Id === delegate.id)) {
                      return prev.map((item) =>
                        item.delegate_Id === delegate.id
                          ? { ...item, portion: value }
                          : item
                      );
                    }

                    // Otherwise, add the delegate with the new portion
                    return [
                      ...prev,
                      { delegate_Id: delegate.id, portion: value },
                    ];
                  });
                }
              }}
    ...
    [delegatesData, query.delegatesPortions, handleRowSelection, isRowSelected]
  );
