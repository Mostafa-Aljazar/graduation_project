'use client';
import {
  delegatesFilterSchema,
  delegatesFilterValues,
} from '@/validation/actor/general/delegates-filter-form';
import {
  Button,
  Flex,
  Group,
...
interface DelegateFiltersProps {
  setLocalFilters: React.Dispatch<React.SetStateAction<delegatesFilterValues>>;
  delegatesNum: number;
}

// FIXME: get max data from API.
const initData: delegatesFilterValues = {
  displaceds_number: [0, 1000], //TODO: get max
  tents_number: [0, 200], //TODO: get max
};
...
export default function Delegate_Filters({
  ...
  const form = useForm<delegatesFilterValues>({
    initialValues: initData,
    validate: zodResolver(delegatesFilterSchema),
  });
